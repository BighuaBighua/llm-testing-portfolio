...
`export_csv()` 用 pandas 导出 `bad_cases.csv`，包含核心字段（case_id、severity、dimension等）、内容字段（input、actual_response等）、根因字段（root_cause_category等），方便在Excel中筛选分析。
...
| `bad_cases.csv` | `cases/bad_cases/bad_cases.csv` | CSV 导出（pandas） |
