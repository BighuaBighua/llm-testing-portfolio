...
ConfigRegistry
    ...
    └── 执行配置 (execution.yaml)
        ├── concurrency (并发模式)
        ├── parameters (超时/重试/限流/推理参数)
        │   ├── timing (api_timeout, case_timeout)
        │   ├── retry (max_attempts, backoff_factor, max_delay, status_forcelist, no_retry_status_codes)
        │   ├── rate_limit (delay_between_calls, delay_on_429)
        │   └── inference (under_test, evaluator)
        └── quality_gate (质量门禁)
```
...
| `retry_config` | dict | 重试配置（max_attempts, backoff_factor, max_delay, status_forcelist, no_retry_status_codes） |
| `rate_limit_config` | dict | 限流配置（delay_between_calls, delay_on_429） |
