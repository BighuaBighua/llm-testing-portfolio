...
| 工程化能力 | 全流程自动化 + 批次管理 + 审计报告 + Bad Case管理 + 项目脚手架 + API统一客户端 | `run_tests.py`, `reporting.py`, `init_project.py`, `api_client.py` |
...
│   └── tools/                            # 工具模块（9个）
│       ├── api_client.py                 # API统一客户端（重试/限流/日志/异常捕获）
...
**两阶段调用流程**：
1. **阶段1**：通过 `APIClient.post()` 调用被测模型（千帆）获取 AI 回答
2. **阶段2**：通过 `APIClient.call_openai()` 调用评测模型（DashScope/ModelScope/千帆兜底）评测回答质量

两阶段调用统一经过 `APIClient`，共享重试、限流、日志、异常捕获逻辑。
...
- APIClient 统一 API 调用（重试/限流/日志/异常捕获）

### API 统一客户端模块 (`scripts/tools/api_client.py`)

**类**：`APIClient`

**核心职责**：统一所有 API 调用的重试、限流、日志、异常捕获，保证调用出口一致。

| 方法 | 职责 | 适用场景 |
|------|------|---------|
| `post(url, headers, json_body)` | 统一 POST 请求（指数退避重试 + 状态码分级 + 异常捕获） | 被测模型调用 |
| `call_openai(prompt, provider)` | OpenAI 兼容调用（手动重试 + 指数退避 + 日志） | 评测模型调用 |
| `_rate_limit()` | 请求前限流（基于 `delay_between_calls` 配置） | 所有 API 调用 |
| `_calculate_backoff(attempt)` | 计算指数退避等待时间 | 重试逻辑 |

**HTTP 状态码分级处理**：
- `400/401/403`：客户端错误，不重试，记录日志
- `429`：限流，额外等待 `delay_on_429` 后重试
- `500/502/503/504`：服务端错误，指数退避重试

**双轨日志机制**：
- `print`：终端实时展示（进度、重试、成功/失败）
- `logger`：结构化日志留痕（异常堆栈、调试信息）

**配置驱动**：所有参数从 `execution.yaml` 读取，通过 `ConfigRegistry.retry_config` 和 `ConfigRegistry.rate_limit_config` 注入。
...
**问题**：被测模型和评测模型的 API 调用逻辑分散在 TestRunner 中，重试/限流/异常处理规则不一致，YAML 配置未生效

**方案**：新建 `APIClient` 类，统一所有 API 调用出口
- **指数退避重试**：`wait = min(backoff_factor × 2^attempt, max_delay)`，参数从 `execution.yaml` 读取
- **请求前限流**：基于 `delay_between_calls` 配置，每次 API 调用前 `time.sleep` 防止触发限流
- **HTTP 状态码分级**：400/401/403 不重试，429 额外等待后重试，5xx 指数退避重试
- **双轨日志**：`print` 终端展示进度，`logger` 结构化日志留痕
- **统一异常捕获**：ConnectionError / Timeout / HTTPError / JSONDecodeError / Exception 分层处理
...
| `execution.yaml` | 执行参数配置 | concurrency(3种模式), parameters(超时/重试/限流/推理参数), quality_gate, performance_monitoring |
...
| V3.2 | 2026-04-27 | APIClient 统一调用客户端（指数退避重试/请求前限流/HTTP状态码分级/双轨日志/统一异常捕获），ConfigRegistry 新增 retry_config/rate_limit_config | API 调用统一出口 + 配置驱动 |
